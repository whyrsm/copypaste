...
describe('insertPaste / getPaste', () => {
  test('insertPaste returns true on success', () => {
    const ok = insertPaste('test-slug-1', 'hello world', null, null, 'plaintext', 'token-abc');
    expect(ok).toBe(true);
  });

  test('getPaste returns the inserted paste with correct fields', () => {
    insertPaste('test-slug-fields', 'content here', null, null, 'javascript', 'token-fields');
  ...
  test('getPaste returns all expected fields', () => {
    insertPaste('full-fields-slug', 'full content', null, null, 'python', 'token-full');
    const paste = getPaste('full-fields-slug');
    expect(paste).not.toBeNull();
    const fields: (keyof typeof paste)[] = [
      'id', 'slug', 'content', 'password_hash', 'views', 'copies',
      'created_at', 'expires_at', 'language', 'author_token',
    ];
