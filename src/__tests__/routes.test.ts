...
describe("POST /api/paste", () => {
  ...
  test("custom slug redirects to that slug", async () => {
    const res = await formReq("/api/paste", {
      content: "custom slug paste",
      slug: "my-custom-slug",
    });
    expect(res.status).toBe(303);
    const location = res.headers.get("location");
...
describe("GET /:slug", () => {
  test("returns 200 with HTML containing the slug for existing paste", async () => {
    ...
    const createRes = await formReq("/api/paste", {
...
describe("POST /:slug/copy", () => {
  ...
  test("returns 404 for non-existent slug", async () => {
    const res = await req("/does-not-exist-xyz/copy", { method: "POST" });
    expect(res.status).toBe(404);
  });
});
