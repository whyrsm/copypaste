...
import { LANGUAGE_LABELS, LANGUAGES, type Language } from "./languages";
...
export function pastePage(
  ...
  const expires = paste.expires_at
    ...
    : null;
  const language =
    paste.language && (LANGUAGES as readonly string[]).includes(paste.language)
  ...
  return layout(
    ...
    html`
      ...
      ${raw(
        `<script id="paste-data" type="application/json">${JSON.stringify({
              ...
              <table class="paste-table">
                ...
                <tbody>
                  ${pastes.map((p) => {
                    ...
                    const lang =
                      p.language &&
                      (LANGUAGES as readonly string[]).includes(p.language)
